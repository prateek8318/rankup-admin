...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Admin Users service
      '/api/admin/users': {
        target: 'http://192.168.1.5:5002', // User Service port
        changeOrigin: true,
        secure: false,
        configure: (proxy, options) => {
          proxy.on('error', (err, req, res) => {
      ...
      // Admin Exams service
      '/api/exams': {
        target: 'http://192.168.1.5:5009', // Correct port where ExamService is running
        changeOrigin: true,
