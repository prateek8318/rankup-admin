...
export default defineConfig({
  ...
  server: {
    proxy: {
      // Auth service
      '/api/admin/auth': {
        target: 'http://192.168.1.32:56924',
        changeOrigin: true,
        secure: false,
      ...
      '/api/admin/subscription-plans': {
        target: 'http://192.168.1.32:56925',
        changeOrigin: true,
        secure: false,
        configure: (proxy, options) => {
          proxy.on('error', (err, req, res) => {
      ...
      // Admin Users service
      '/api/admin/users': {
        target: 'http://192.168.1.32:5002', // User Service port
        changeOrigin: true,
        secure: false,
        configure: (proxy, options) => {
          proxy.on('error', (err, req, res) => {
      ...
      // Admin Exams service
      '/api/exams': {
        target: 'http://192.168.1.32:5009', // Correct port where ExamService is running
        changeOrigin: true,
      ...
      '/api/qualifications': {
        ...
        configure: (proxy, options) => {
          proxy.on('error', (err, req, res) => {
            console.log('qualifications proxy error', err);
          });
          proxy.on('proxyReq', (proxyReq, req, res) => {
            console.log('Sending Qualifications Request to the Target:', req.method, req.url);
          });
          proxy.on('proxyRes', (proxyRes, req, res) => {
            console.log('Received Qualifications Response from the Target:', proxyRes.statusCode, req.url);
          });
        }
      },
      '/api/streams': {
        ...
        configure: (proxy, options) => {
          proxy.on('error', (err, req, res) => {
            console.log('streams proxy error', err);
          });
          proxy.on('proxyReq', (proxyReq, req, res) => {
            console.log('Sending Streams Request to the Target:', req.method, req.url);
          });
          proxy.on('proxyRes', (proxyRes, req, res) => {
